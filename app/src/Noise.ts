/**
 * This is Joseph Gentle implementation converted into a module,
 * with an added Octave methods
 * 
 * reference article
...
export default class Noise {
  ...
  /**
   * Generate 2D Perlin noise using octaves and persistence
   * 
   * @param {number} x
   * @param {number} y
   * @param {number} octaves Number of time to run Perlin
   * @param {number} persistence Amount of influence each successive octave has
   * @returns {number} -1 to 1
   */
  Perlin2DWithOctaves = (x: number, y: number, octaves: number = 1, persistence: number = .5): number => {
    let total = 0
    let frequency = 1
    let amplitude = 1
    let maxValue = 0
